      ...
      <div className="bg-card rounded-lg shadow-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          {PAYMENT_TYPES.map((pt) => {
            ...
            return (
              <div key={pt.type} className={`border rounded-lg p-4 transition-colors ${method.enabled ? 'border-primary bg-primary/5' : 'border-border'}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button type="button" onClick={() => updateMethod(pt.type, 'enabled', !method.enabled)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${method.enabled ? 'bg-primary' : 'bg-muted'}`}>
                ...
                {method.enabled && (
                  <input type="text" value={method.value} onChange={(e) => updateMethod(pt.type, 'value', e.target.value)} placeholder={pt.placeholder} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
                )}
              </div>
            )
          })}
